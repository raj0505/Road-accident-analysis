...
{ // begin of form
		...
		try
		{
			frame = new JFrame("Category Master Form");
    			...
    			Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
                       img = new JLabel("");
                       img.setIcon(new ImageIcon("t.jpg"));
			...
			frame.setBounds(0,0,screenSize.width,screenSize.height);
		   	...
		   	lblHeading = new JLabel("The Information Of The Accident Area");
			...
			lblHeading.setFont(new Font("Comic Sans MS", Font.BOLD, 23));
			...
			addComponentToPane(frame.getContentPane(),btnExit,340,500,20,18,"exit");
                         addComponentToPane(frame.getContentPane(),img,60,50,500,100);
		}
		catch(Exception e)
		{
	...
	private static void prepare_table_data()
	{
		...
		try
		{
			...
			Statement stmt1 = con.createStatement();
	        	String tableName ="situation_detail";
	        	String sql = "select  acc_year,accidents_type,situation,directions from situation_detail"; 
	    		...
	    		while (res.next()) 
			{
				...
				newrow.addElement(res.getObject("acc_year"));
                                newrow.addElement(res.getObject("accidents_type"));
                                newrow.addElement(res.getObject("situation"));
				newrow.addElement(res.getObject("directions"));
	...
	private static void display_data_in_table()
	{
		...
		cname.addElement("acc_year");
               ...
               // Vector cname = new Vector();
                cname.addElement("accidents_type");
                cname.addElement("situation");
		cname.addElement("directions");
...
public static void main(String s[]) 
{
    javax.swing.SwingUtilities.invokeLater(new Runnable() 
   {
	 public void run() 
	{
		display9_hive h=new display9_hive();
		h.createAndShowGUI();
	}
});
