...
{ // begin of form
		...
		try
		{
			...
			frame.setBounds(0,0,screenSize.width,screenSize.height);
		    	...
		    	frame.getRootPane().setWindowDecorationStyle(JRootPane.NONE);
                        img = new JLabel("");
                        img.setIcon(new ImageIcon("p7.jpg"));
		    	frame.setVisible(true);
		   	lblHeading = new JLabel("The maximum type of vehicle model crashed in accidents ");
			...
			lblHeading.setFont(new Font("Comic Sans MS", Font.BOLD, 20));
			...
			addComponentToPane(frame.getContentPane(),btnExit,340,500,20,22,"exit");
                        addComponentToPane(frame.getContentPane(),img,-50,-20,10,50);
		}
		catch(Exception e)
		{
	...
	private static void prepare_table_data()
	{
		...
		try
		{
			...
			Statement stmt1 = con.createStatement();
	        	String tableName ="vehicle_design1";
	        	String sql = "select  acc_year, body_style, model, color from vehicle_design1"; 
	    		...
	    		while (res.next()) 
			{
				...
				newrow.addElement(res.getObject("acc_year"));
                                newrow.addElement(res.getObject("body_style"));
                                newrow.addElement(res.getObject("model"));
				newrow.addElement(res.getObject("color"));
	...
	private static void display_data_in_table()
	{
		...
		cname.addElement("acc_year");
               ...
               // Vector cname = new Vector();
                cname.addElement("body_style");
                cname.addElement("model");
		cname.addElement("color");
...
public static void main(String s[]) 
{
    javax.swing.SwingUtilities.invokeLater(new Runnable() 
   {
	 public void run() 
	{
		display7_hive h=new display7_hive();
		h.createAndShowGUI();
	}
});
